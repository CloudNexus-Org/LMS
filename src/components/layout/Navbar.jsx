  ...
  return (
    ...
    >
      ...
      {items.map((item) => (
        ...
        >
          <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-primary/10 text-primary transition-colors group-hover:bg-primary/15 group-hover:text-primary">
            {item.icon}
          </span>
    ...
    <>
      ...
      >
        <div
          ...
          style={{ backgroundColor: isLandingTop ? "transparent" : bgColor }}
        />

        <div className={`relative z-10 mx-auto flex h-16 w-full items-center ${SHELL_MAX_WIDTH} ${SHELL_PADDING}`}>
          <Link
            to="/"
            onClick={() => {
              setActiveSection("");
            ...
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  {hasDropdown ? (
                    <button
                      ...
                      className={`
                        ...
                        ${isActive || isOpen ? "text-text" : "text-muted hover:text-text"}
                      `}
                    >
                      {link.label}
                      <ChevronDown
                  ...
                  <AnimatePresence>
                    {hasDropdown && isOpen && (
                      <DropdownPanel items={link.items} onSelect={handleNavClick} />
                    )}
                  </AnimatePresence>
                </div>
              );
          ...
          <div className="relative z-10 ml-auto flex items-center gap-2.5 sm:gap-3 lg:gap-4">
            ...
            <button
              ...
              style={{
                backgroundColor: isLandingTop ? "transparent" : bgColor,
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            ...
            >
              <div
                className={`
                  relative overflow-hidden rounded-b-2xl border-t backdrop-blur-2xl
                  ${isDarkTheme ? "border-white/10 bg-[#050816]/98" : "border-black/10 bg-white/98"}
                `}
              >
                      ...
                      const isActive = hasDropdown
                        ? link.items.some(
                            (item) =>
                              ...
                              (item.href === "/mentors" && location.pathname === "/mentors")
                          )
                      ...
                      if (!hasDropdown) {
                        return (
                          <motion.a
                            ...
                            onClick={(e) => handleNavClick(e, link.href)}
                            className={`
                              flex items-center gap-3 rounded-xl border px-5 py-4 text-[15px] font-medium transition-colors
                              ${isDarkTheme ? "border-white/10 bg-white/[0.03] text-white" : "border-black/10 bg-black/[0.02] text-black"}
                        ...
                        >
                          <button
                            ...
                            className={`
                              ...
                              ${isDarkTheme ? "bg-white/[0.03] text-white" : "bg-black/[0.02] text-black"}
