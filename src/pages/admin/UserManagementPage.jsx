    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-bold text-text font-display tracking-tight">Platform Users</h1>
          <p className="text-muted mt-1 font-medium">Manage students, mentors, and platform administrators.</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="inline-flex items-center gap-2 px-4 py-2.5 bg-surface border border-border rounded-xl text-sm font-bold text-muted hover:text-text hover:border-primary/40 transition-all shadow-sm">
            <Download className="h-4 w-4" /> Export
          </button>
          <button className="inline-flex items-center gap-2 px-4 py-2.5 bg-primary text-white rounded-xl text-sm font-bold shadow-sm hover:bg-primary-hover hover:-translate-y-0.5 transition-all">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          const Icon = kpi.icon;
          return (
            <div key={kpi.label} className="bg-surface border border-border rounded-2xl p-5 shadow-sm hover:-translate-y-0.5 hover:shadow-md transition-all duration-300">
              <div className="flex items-center justify-between mb-3">
                <div className={`h-9 w-9 rounded-xl ${kpi.bg} ${kpi.color} flex items-center justify-center`}>
                  <Icon className="h-4 w-4" />
                </div>
                {kpi.trend && (
                  <span className="text-[10px] font-bold text-success bg-success/10 px-2 py-0.5 rounded-lg flex items-center gap-0.5">
      ...
      <div className="bg-surface border border-border rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-5 border-b border-border bg-bg/30">
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              ...
              className="w-full h-10 rounded-xl border border-border bg-surface pl-10 pr-8 text-sm font-medium text-text focus:border-primary focus:ring-1 focus:ring-primary/20 outline-none transition-all"
            />
            {search && (
          ...
          <div className="flex items-center gap-1 bg-bg border border-border rounded-xl p-1">
            {['All', 'Student', 'Mentor', 'Admin'].map(r => (
              <button key={r} onClick={() => setRoleFilter(r)}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${roleFilter === r ? 'bg-primary text-white shadow-sm' : 'text-muted hover:text-text'}`}>
                {r}
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center gap-1 bg-bg border border-border rounded-xl p-1">
            {['All', 'Active', 'Inactive', 'Banned'].map(s => (
              <button key={s} onClick={() => setStatusFilter(s)}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${statusFilter === s ? 'bg-primary text-white shadow-sm' : 'text-muted hover:text-text'}`}>
                {s}
              </button>
            ))}
          </div>
              ...
              {filtered.map((user, i) => {
                ...
                return (
                  <tr key={user.id} className="group hover:bg-bg/40 transition-colors">
                    ...
                    {/* Role */}
                    <td className="px-5 py-4">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold border ${rc.bg} ${rc.border} ${rc.color}`}>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => setSelectedUser(user)} className="h-8 w-8 flex items-center justify-center rounded-xl border border-border text-muted hover:text-primary hover:border-primary/40 hover:bg-primary/5 transition-all" title="View">
                          <Eye className="h-3.5 w-3.5" />
                        </button>
                        <button className="h-8 w-8 flex items-center justify-center rounded-xl border border-border text-muted hover:text-primary hover:border-primary/40 hover:bg-primary/5 transition-all" title="Edit">
                          <Edit3 className="h-3.5 w-3.5" />
                        </button>
                        <button className="h-8 w-8 flex items-center justify-center rounded-xl border border-border text-muted hover:text-danger hover:border-danger/40 hover:bg-danger/5 transition-all" title={user.status === 'Banned' ? 'Unban' : 'Ban'}>
        ...
        <div className="px-5 py-3 border-t border-border bg-bg/30 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            {[1, 2, 3].map(p => (
              <button key={p} className={`h-7 w-7 rounded-lg text-xs font-bold transition-all ${p === 1 ? 'bg-primary text-white' : 'text-muted hover:bg-bg border border-border hover:text-text'}`}>{p}</button>
            ))}
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setSelectedUser(null)}>
          <div className="bg-surface border border-border rounded-2xl shadow-elevated w-full max-w-md p-6 animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              ...
              <button className="flex items-center justify-center gap-2 px-4 py-2.5 bg-danger/10 border border-danger/20 rounded-xl text-sm font-bold text-danger hover:bg-danger hover:text-white transition-all">
